...
import { geniusContent, type Locale } from './content'
...
/* Cinematic looping backdrop; falls back to the poster frame when the
   ...
   Assets are reused from the /fifa brief. */
function HeroVideo() {
  ...
  return (
    <div className="absolute inset-0">
      ...
      <div className="absolute inset-0 bg-[#080a18]/40" />
      <div className="absolute inset-0 bg-gradient-to-t from-[#080a18] via-[#080a18]/25 to-[#080a18]/55" />
    </div>
  )
}
...
export default function GeniusFitPage() {
  const [locale, setLocale] = useState<Locale>('en')
  const [filter, setFilter] = useState<MatchFilter>(null)
  const t = geniusContent[locale]
      ...
      <section className="relative flex min-h-[100svh] flex-col overflow-hidden">
        ...
        <div className="relative mx-auto flex w-full max-w-6xl flex-1 flex-col px-4 pt-10 pb-28 sm:px-6 md:pb-32 lg:px-8">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <p className="section-label" style={{ color: '#aeb6ff' }}>
              {t.hero.eyebrow}
            </p>
