      ...
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-6xl mx-auto">
          <div className="text-center py-12">
            <p className="text-slate-600 dark:text-slate-400">Failed to load blog content.</p>
          </div>
        </div>
      </div>
    )
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <section className="text-center mb-16">
          ...
          <p className="text-xl text-slate-600 dark:text-slate-400 max-w-3xl mx-auto mb-8">
            {content.hero?.description || 'Real stories and lessons learned from 8+ years managing international teams at adidas Digital Sports and fintech startups.'}
          </p>
            ...
            <div className="grid lg:grid-cols-2 gap-12 items-center">
              <div>
                ...
                <h2 className="text-3xl lg:text-4xl font-bold mb-6">
                  {content.featured?.title || 'Managing International Teams: Lessons from Leading 13 People Across Multiple Time Zones'}
                </h2>
                <p className="text-lg text-slate-600 dark:text-slate-300 mb-6">
                  {content.featured?.description || 'Real insights from my experience leading cross-functional teams of up to 13 personnel at adidas Digital Sports.'}
                </p>
                ...
                <Card>
                  ...
                  <CardContent>
                    <ul className="text-sm space-y-2 text-slate-600 dark:text-slate-400">
                      {content.featured?.keyInsights?.map((insight: string, index: number) => (
                        <li key={index}>
                          • {insight}
                        </li>
                      )) || [
                        <li key="1">• Communication strategies across multiple time zones</li>,
                        <li key="2">• Managing external freelancers and internal teams</li>,
                        <li key="3">• Coordinating complex technical migrations with zero downtime</li>,
                        <li key="4">• Building trust and accountability in distributed teams</li>
                      ]}
                    </ul>
                  </CardContent>
                </Card>
        ...
        <section className="mb-16">
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            {content.articles?.map((article: any, index: number) => (
              ...
              // Fallback content
              <Card key="fallback-1" className="group hover:shadow-lg transition-shadow">
                <CardHeader>
                  <div className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400 mb-2">
                    <Calendar className="h-4 w-4" />
                  ...
                  <CardDescription className="text-base">
                    How I went from Sport Management graduate to leading international product teams at adidas Digital Sports serving 165M+ users.
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <p className="text-slate-600 dark:text-slate-400 mb-4">
                    After leading 15+ cross-functional teams, I've learned that great collaboration isn't about processes—it's about people.
                  </p>
                  <div className="flex items-center justify-between">
                    <div className="flex gap-2">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {content.expertise?.areas?.map((area: any, index: number) => {
              ...
              const getIcon = (title: string) => {
                if (title.toLowerCase().includes('team') || title.toLowerCase().includes('leadership')) {
                  return <Users className="h-6 w-6 text-blue-600 dark:text-blue-400" />
                }
                if (title.toLowerCase().includes('sports') || title.toLowerCase().includes('commerce')) {
                  return <Target className="h-6 w-6 text-green-600 dark:text-green-400" />
                }
                if (title.toLowerCase().includes('fintech') || title.toLowerCase().includes('trading')) {
                  return <TrendingUp className="h-6 w-6 text-purple-600 dark:text-purple-400" />
                }
                if (title.toLowerCase().includes('tech') || title.toLowerCase().includes('migration')) {
                  return <Brain className="h-6 w-6 text-orange-600 dark:text-orange-400" />
                }
                if (title.toLowerCase().includes('seo') || title.toLowerCase().includes('marketing')) {
                  return <BarChart3 className="h-6 w-6 text-red-600 dark:text-red-400" />
                }
                if (title.toLowerCase().includes('agile') || title.toLowerCase().includes('strategic')) {
                  return <Lightbulb className="h-6 w-6 text-teal-600 dark:text-teal-400" />
                }
              ...
              // Fallback content
              <Card key="fallback-1" className="text-center group hover:shadow-md transition-shadow">
                <CardHeader>
