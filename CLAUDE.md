...
- **Every change applies to every locale.** Both the refine box (`POST /api/admin/brief/[id]/refine`, before saving) and a hand edit in the JSON editor (`POST /api/admin/brief/[id]/mirror`, after the PUT) **mirror** into the other languages through `src/lib/fit-brief/mirror.ts`: each locale gets its own call with its current text plus the fields that moved, and returns everything else byte-identical. Not a re-translation — that would overwrite hand edits in IT/DE. One Accept writes them through the ordinary PUT. What counts as a change is every string leaf (`mirror-diff.ts`), not the learning loop's prose-only paths, or a stat label would never travel; and `alignFactIds()` makes a mirror incapable of changing a citation, which the model will otherwise do when an edit leaves a label and its number disagreeing. A locale that fails is named and skipped; one that does not exist is skipped silently. Before this, a change landed in one language and the other two silently drifted.
