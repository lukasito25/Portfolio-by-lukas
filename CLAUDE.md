...
- **Five rules for the documents:**
  ...
  - **`highlights` on `CvContentSchema` must stay `.default([])`.** It feeds the stat band. Required would break five `safeParse` call sites for every CV generated before it existed — two of them silently.
