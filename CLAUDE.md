...
- **Storage**: `GeneratedBrief` and `JobLead` — local SQLite in development, D1 elsewhere. Deliberately unlike `/api/campaigns`, which keys on `API_SECRET`: a brief drafted locally must not land in production D1 one click from being published.
