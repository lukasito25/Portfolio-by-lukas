...
- **A refinement applies to every locale.** `POST /api/admin/brief/[id]/refine` proposes in the locale on screen, then **mirrors** the change into the others — each gets its own call with its current text plus the fields that moved, and returns everything else byte-identical. Not a re-translation: that would overwrite hand edits in IT/DE. One Accept writes them all through the ordinary PUT. A locale that fails is named and skipped, one that does not exist is skipped silently. Before this, a refinement landed in one language and the other two silently drifted.
