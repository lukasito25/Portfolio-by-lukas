...
- **Output**:
  ...
  - A CV and cover letter as **.docx** downloads, rendered on demand from `templates/*.docx`. Two buttons: **.docx** for an ATS portal and **.pdf** for a person — see "Two formats, two readers" in `CUSTOM_RECRUITER_PAGES.md`. Each button names the design it will download. By default that is the recommendation for this posting — a design per format, `classic` .docx and `column` PDF unless the sector or the country says otherwise — and **Which document to send** under the fit score says which to attach and why (the tracking system, the country, the sector). The picker forces one design for both formats — see "Document designs" in `CUSTOM_RECRUITER_PAGES.md`. When the PDF is `column`, a **Photo in PDF** checkbox decides per download whether the headshot is on it (a UK or US recruiter usually should not get one); it needs `templates/photo.jpg` in the deployment and says so when there is none. All are the same document to an ATS except `column`, the two-column one, which is read rail-first in .docx; as a PDF, `column` is the dark-rail page modelled on the résumé he sent by hand, drawn so a parser reads name and contact first
...
#### 4d. 🔎 Search for jobs (`/admin/applications` → Search tab)

- **Purpose**: find the postings in the first place, instead of trawling LinkedIn and StepStone by hand
...
- **Badges**, all of them checked rather than claimed:
  ...
  - **Could not verify** — a timeout or a bot wall. Its own answer, never collapsed into "gone", because LinkedIn and Indeed refuse server-side requests as a matter of course
