...
#### Private recruiter fit-brief pages (`/fifa`, `/genius`, `/qualcomm`, `/archlet`, `/launchmetrics`, `/qonto`, `/kraken`, `/ubp`, `/scandit`, `/zalando`, `/rocken`)

- **Role-tailored, `noindex`, unlisted** (not in nav or sitemap) — shared by direct link alongside a CV
- **In-page language toggle**, chosen per role (EN/IT/DE, EN/DE/ES/IT on `/archlet`, EN/FR/IT/ES on `/launchmetrics`, EN/FR/IT/DE/ES on `/qonto`, EN/DE/IT on `/kraken` and `/scandit`, EN/DE/FR on `/ubp`, EN/DE on `/zalando`); cinematic hero; interactive requirement-match filter
- **A brief can lead in a language other than English** — `/rocken` opens in **German** (DE/EN) because the posting, the customers (cantonal tax administrations) and the working groups all run in German. It also syncs `document.documentElement.lang` to the active locale, since the root layout hard-codes `lang="en"`
- **Hero is video or code-generated** — `/fifa` and `/genius` use a looping clip; `/qualcomm` renders an animated SVG circuit board, `/archlet` a bid lattice, `/launchmetrics` a data-flow pipeline, `/qonto` a transaction categoriser, `/kraken` an any-to-any conversion mesh and `/ubp` concentric data-classification access rings, none of which ship binary assets. `/scandit` and `/zalando` are the exceptions: they reuse the **homepage hero composition** (`HeroCanvas` particle field + vignette + grain)
- **Per-page brand accent** (`/genius` electric blue, `/qualcomm` Arduino teal, `/archlet` indigo, `/launchmetrics` mint, `/qonto` yellow, `/kraken` purple, `/ubp` UBP teal-green with a gold core) via a scoped `[data-brand]` override — **optional**: `/scandit` and `/zalando` deliberately carry no override and run on the site's own Ink & Signal palette
- **Scannable layout variant** — `/zalando` trades long prose for interaction: the role map is a **swipeable card deck** (native scroll-snap, arrows, dot indicators), the spotlight is a lede plus four numbered pillar cards, and the profile-match panels are **tabbed** so one set of rows shows at a time. Use it when the reader is senior and the page would otherwise run long
- **Tone is set per company** — `/kraken` leads on a prototyping work sample, `/ubp` runs deliberately restrained for a family-owned private bank and carries the most transferable rows of any brief (14 direct / 6 transferable)
