...
#### Private recruiter fit-brief pages (`/fifa`, `/genius`, `/qualcomm`, `/archlet`, `/launchmetrics`, `/qonto`, `/kraken`, `/ubp`, `/scandit`)

- **Role-tailored, `noindex`, unlisted** (not in nav or sitemap) — shared by direct link alongside a CV
- **In-page language toggle**, chosen per role (EN/IT/DE, EN/DE/ES/IT on `/archlet`, EN/FR/IT/ES on `/launchmetrics`, EN/FR/IT/DE/ES on `/qonto`, EN/DE/IT on `/kraken` and `/scandit`, EN/DE/FR on `/ubp`); cinematic hero; interactive requirement-match filter
- **Hero is video or code-generated** — `/fifa` and `/genius` use a looping clip; `/qualcomm` renders an animated SVG circuit board, `/archlet` a bid lattice, `/launchmetrics` a data-flow pipeline, `/qonto` a transaction categoriser, `/kraken` an any-to-any conversion mesh and `/ubp` concentric data-classification access rings, none of which ship binary assets. `/scandit` is the exception: it reuses the **homepage hero composition** (`HeroCanvas` particle field + vignette + grain)
- **Per-page brand accent** (`/genius` electric blue, `/qualcomm` Arduino teal, `/archlet` indigo, `/launchmetrics` mint, `/qonto` yellow, `/kraken` purple, `/ubp` UBP teal-green with a gold core) via a scoped `[data-brand]` override — **optional**: `/scandit` deliberately carries no override and runs on the site's own Ink & Signal palette
